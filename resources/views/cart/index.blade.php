    ...
    $(document).ready(function () {
        ...
        $('.btn-remove').click(function () {
            ...
            .then(function(willDelete) {
                // 用户点击 确定 按钮， willDelete 的值就会是true，否则false
                if (!willDelete) {
                    return;
                }
        ...
        $('#select-all').change(function (){
            // 获取单选框的选中状态
            // prop() 方法可以知道标签中是否包含某个属性,当单选框被勾选时，对应的标签就会新增一个 checked 属性的勾选框
            var checked = $(this).prop('checked');
            console.log(checked);
            // 获取所有 name=select 并且不带有 disable 属性的勾选框
            // 对于已经下架的商品我们希望对应勾选框会被选中，因此我们需要加上 ：not([disable]) 这个条件
            $('input[name=select][type=checkbox]:not([disable])').each(function () {
                // 将其勾选状态设为与目标单选框一致
