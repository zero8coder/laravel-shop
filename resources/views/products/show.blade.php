  ...
  $(document).ready(function () {
    ...
    $('.btn-add-to-cart').click(function () {
      ...
      $('.btn-crowdfunding').click(function () {
          ...
          $form.append('<div class="form-group row">' + '<label class="col-form-label col-sm-3">购买数量</label>' +
              '<div class="col-sm-9"><input class="form-control" name="amount">' + '</div></div>'
          );

          // 调用 SweetAlert 弹框
          swal({
              text: '参与众筹',
              content: $form[0], // 弹窗的内容是刚刚创建的表单
              buttons: ['取消', '确定']
          }).then(function (ret) {
              // 如果用户没有点击确定按钮，则什么也不做
              if (!ret) {
